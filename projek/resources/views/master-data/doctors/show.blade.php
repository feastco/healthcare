...
@section('content')
    <x-common.page-breadcrumb pageTitle="Doctor Detail" />
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <p class="text-theme-sm text-gray-500 dark:text-gray-400">
            Doctor:
            <span class="font-medium text-gray-800 dark:text-white">{{ $doctor->name }}</span>
        </p>
        <div class="flex items-center gap-3">
            ...
            @endcan
            @can('delete', $doctor)
                <x-common.delete-confirmation :action="route('doctors.destroy', $doctor)" label="Delete" />
            @endcan
        </div>
    </div>
    ...
    <div
        ...
        <dl class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
                ...
                <dd class="mt-1 text-theme-sm font-medium text-gray-800 dark:text-white">{{ $doctor->name }}</dd>
            </div>
            <div>
                <dt class="text-theme-xs font-medium uppercase text-gray-500 dark:text-gray-400">Department</dt>
                <dd class="mt-1 text-theme-sm text-gray-700 dark:text-gray-300">{{ $doctor->department?->name }}</dd>
            </div>
            <div>
                <dt class="text-theme-xs font-medium uppercase text-gray-500 dark:text-gray-400">Linked User</dt>
                <dd class="mt-1 text-theme-sm text-gray-700 dark:text-gray-300">{{ $doctor->user?->name }}</dd>
            </div>
            <div>
                ...
                <dd class="mt-1 text-theme-sm text-gray-700 dark:text-gray-300">
                    {{ $doctor->created_at?->format('d M Y H:i') }}</dd>
            </div>
        </dl>
    </div>
@endsection